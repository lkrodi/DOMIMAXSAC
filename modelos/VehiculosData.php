...
class VehiculosData extends Conexion implements consultasExternas
{
	...
	public function listaRegistros()
	{
		...
		if (is_array(self::validaResConsulta($resultado)))
		{
			return $resultado;
		}
	}
	...
	{
		$consulta = $this->mysql->query("SELECT * FROM proforma A INNER JOIN ordenotn B ON A.otn = B.otn");
		$resultado = $consulta->fetchAll();
	...
	public function agregarRegistroProforma($datos=array())
	{
		$consulta = $this->mysql->prepare(
		...
		$consulta->bindParam(":descripciones_p",  $datos[1], PDO::PARAM_STR);
		$consulta->bindParam(":cantidades_p", 	  $datos[2], PDO::PARAM_STR);
		$consulta->bindParam(":precios_p", 		  $datos[3], PDO::PARAM_STR);
	...
	public function buscaUsuario($user, $password)
    {
 	...
 	public function RegistroPdfProformas ($limite)
  	...
  	public function RegistroPdfOtn ($limite)
  	{
	  	$consulta = $this->mysql->query("SELECT * FROM ordenotn LIMIT 0, $limite");
	    $resultado = $consulta->fetchAll();
  	...
  	public function RegistroPdfProforma ($limite)
  	{
	  	$consulta = $this->mysql->query("SELECT * FROM ordenotn LIMIT 0,$limite");
	    $resultado = $consulta->fetchAll();
  	...
  	public function agregaRegistro($datos = array())
	{
		...
		$consulta = $this->mysql->prepare(
			"INSERT INTO ordenotn VALUES(
			:otn, 
			:hora_inicio, 
			:hora_termino, 
			:fecha_dv, 
			:tipomantenimiento_dv,
			:marca_dv, 
			:placa_dv, 
			:modelovehiculo_dv, 
			:modelocarroceria_dv,
	...
	public function actualizaRegistro($datos=array(), $otn)
	{
		$consulta = $this->mysql->prepare(
			"UPDATE ordenotn SET 
				...
				fecha_dv 				= :fecha_dv, 
				tipomantenimiento_dv 	= :tipomantenimiento_dv,
				marca_dv 				= :marca_dv, 
				placa_dv 				= :placa_dv,
				modelovehiculo_dv 		= :modelovehiculo_dv, 
				modelocarroceria_dv 	= :modelocarroceria_dv, 
				color_dv 				= :color_dv, 
				kilometraje_dv 			= :kilometraje_dv, 
				sede_dv 				= :sede_dv, 
				descripcion_dv 			= :descripcion_dv, 
				descripcion_tr 			= :descripcion_tr, 
				descripcion_rau 		= :descripcion_rau, 
				descripcion_st 			= :descripcion_st, 
